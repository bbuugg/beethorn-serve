...
func (s *answerService) SelectList(req *dto.AnswerQueryParam) (int, []*result.AnswerList, response.ResponseCode) {
	...
	list, err := model.
		Fields(
			...
			dao.SysAnswer.Columns.IsAdoption,
	...
	for _, i := range list {
		...
		title, err := dao.SysTopic.Value(dao.SysTopic.Columns.Title, dao.SysTopic.Columns.TopicId, i.TopicId)
		if err != nil {
			return 0, nil, response.DB_READ_ERROR
		}
		info.TopicInfo.Title = gconv.String(title)
		info.TopicInfo.Id = i.TopicId
...
func (s *answerService) Review(req *dto.Review) (code response.ResponseCode) {
	...
	for _, i := range tmpInfoList {
		...
		question, err := dao.SysTopic.
			Fields(dao.SysTopic.Columns.UserId, dao.SysTopic.Columns.Title).
			Where(dao.SysTopic.Columns.TopicId, i.TopicId).One()
		if err != nil {
			return response.DB_READ_ERROR
		}
