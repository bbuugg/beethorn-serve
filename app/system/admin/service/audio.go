...
func (s *audioService) Create(req *dto.AudioCreate) (code response.ResponseCode) {
	...
	if len(req.ResourceId) > 0 {
		err = Resource.AddRelated(tx, req.ResourceId, rid, shared.Audio)
		if err != nil {
			return response.ADD_FAILED
		}
	}
...
func (s *audioService) Edit(req *dto.AudioEdit) (code response.ResponseCode) {
	...
	// 增加 关联资源
	err = Resource.RemoveRelated(tx, []int64{req.AudioId}, shared.Audio)
	if err != nil {
		return response.DELETE_FAILED
	}
	if len(req.ResourceId) > 0 {
		err = Resource.AddRelated(tx, req.ResourceId, req.AudioId, shared.Audio)
		if err != nil {
			return response.ADD_FAILED
		}
	}
...
func (s *audioService) Remove(idList []int64) (code response.ResponseCode) {
	...
	err = Resource.RemoveRelated(tx, idList, shared.Audio)
	if err != nil {
		return response.DELETE_FAILED
	}
