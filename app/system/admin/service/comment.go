...
func (s *commentService) SelectList(req *dto.CommentQueryParam) (int, []*result.CommentList, response.ResponseCode) {
	...
	for _, i := range list {
		...
		switch i.Module {
		case shared.Article:
			title, err := dao.SysArticle.Value(dao.SysArticle.Columns.Title, dao.SysArticle.Columns.ArticleId, i.RelatedId)
			if err != nil {
				return 0, nil, response.DB_READ_ERROR
			}
			info.RelatedInfo.Title = gconv.String(title)
			info.RelatedInfo.Id = i.RelatedId
			info.RelatedInfo.Module = i.Module
		case shared.Audio:
			title, err := dao.SysAudio.Value(dao.SysAudio.Columns.Title, dao.SysAudio.Columns.AudioId, i.RelatedId)
			if err != nil {
				return 0, nil, response.DB_READ_ERROR
			}
			info.RelatedInfo.Title = gconv.String(title)
			...
			info.RelatedInfo.Module = i.Module
		case shared.Video:
			title, err := dao.SysVideo.Value(dao.SysVideo.Columns.Title, dao.SysVideo.Columns.VideoId, i.RelatedId)
			if err != nil {
				return 0, nil, response.DB_READ_ERROR
			}
			info.RelatedInfo.Title = gconv.String(title)
			...
			info.RelatedInfo.Module = i.Module
		case shared.Edu:
			title, err := dao.SysEdu.Value(dao.SysEdu.Columns.Title, dao.SysEdu.Columns.EduId, i.RelatedId)
			if err != nil {
				return 0, nil, response.DB_READ_ERROR
			}
			info.RelatedInfo.Title = gconv.String(title)
