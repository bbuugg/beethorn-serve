...
func (s *videoService) Create(req *dto.VideoCreate) (code response.ResponseCode) {
	...
	if len(req.ResourceId) > 0 {
		err = Resource.AddRelated(tx, req.ResourceId, rid, shared.Video)
		if err != nil {
			return response.ADD_FAILED
		}
	}
...
func (s *videoService) Edit(req *dto.VideoEdit) (code response.ResponseCode) {
	...
	// 增加 关联资源
	err = Resource.RemoveRelated(tx, []int64{req.VideoId}, shared.Video)
	if err != nil {
		return response.DELETE_FAILED
	}
	if len(req.ResourceId) > 0 {
		err = Resource.AddRelated(tx, req.ResourceId, req.VideoId, shared.Video)
		if err != nil {
			return response.ADD_FAILED
		}
	}
...
func (s *videoService) Remove(idList []int64) (code response.ResponseCode) {
	...
	// 删除 关联资源
	err = Resource.RemoveRelated(tx, idList, shared.Video)
	if err != nil {
		return response.DELETE_FAILED
	}
	return response.SUCCESS
