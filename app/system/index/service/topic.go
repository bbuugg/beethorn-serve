	...
	case shared.ModeFollow:
		if req.UserId != 0 {
			...
			model = model.Where(dao.SysTopic.Columns.UserId+" IN(?)", followIdList).
				Or(dao.SysTopic.Columns.UserId, req.UserId).Order(dao.SysTopic.Columns.CreateTime + " desc")
...
func (s *topicService) SelectInfo(userId, id int64) (*result.TopicInfo, response.ResponseCode) {
	...
	redisCom.Key = shared.Topic + shared.InfoById + gconv.String(id) + gconv.String(userId)
	// 获取缓存
...
func (s *topicService) info(userId int64, info *model.SysTopic) (*result.TopicInfo, error) {
	...
	if info.Module != "" && info.RelatedId != 0 {
		var relatedInfo result.RelatedInfo
		switch info.Module {
		case shared.Article:
			tmpInfo, err := dao.SysArticle.
				Fields(dao.SysArticle.Columns.Title, dao.SysArticle.Columns.Description, dao.SysArticle.Columns.Cover).
				Where(dao.SysArticle.Columns.ArticleId, info.RelatedId).
				One()
			if err != nil {
				return nil, err
			}
			relatedInfo.Description = tmpInfo.Description
		...
		case shared.Audio:
			tmpInfo, err := dao.SysAudio.
				Fields(dao.SysAudio.Columns.Title, dao.SysAudio.Columns.Description, dao.SysAudio.Columns.Cover).
		...
		case shared.Video:
			tmpInfo, err := dao.SysVideo.
				Fields(dao.SysVideo.Columns.Title, dao.SysVideo.Columns.Description, dao.SysVideo.Columns.Cover).
		...
		case shared.Resource:
			tmpInfo, err := dao.SysResource.
				Fields(dao.SysResource.Columns.Title, dao.SysResource.Columns.Description, dao.SysResource.Columns.Cover).
		...
		case shared.Edu:
			tmpInfo, err := dao.SysEdu.
				Fields(dao.SysEdu.Columns.Title, dao.SysEdu.Columns.Description, dao.SysEdu.Columns.Cover).
