...
func (s *resourceService) SelectInfo(userId, id int64) (*result.ResourceInfo, response.ResponseCode) {
	...
	redisCom.Key = shared.Resource + shared.InfoById + gconv.String(id) + gconv.String(userId)
	// 获取缓存
...
func (s *resourceService) RemoveRelated(tx *gdb.TX, relatedId int64, module string) error {
	_, err := tx.Model(dao.SysResourceRelated.Table).
		Where(dao.SysResourceRelated.Columns.RelatedId, relatedId).
...
func (s *resourceService) SelectRelatedList(rid int64, module string) ([]*result.ResourceList, error) {
	var listId []int64
	...
	var list []*result.ResourceList
	err = dao.SysResource.Where(dao.SysResource.Columns.ResourceId+" IN(?)", listId).Structs(&list)
	if err != nil {
		return nil, err
	}
	return list, nil
